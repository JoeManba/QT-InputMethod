...
{
    //每次匹配中文都先清空之前的列表
    tempChineseList.clear();
    //谷歌拼音
    googlePinyin->SearchCN(pinyin);
    tempChineseList = googlePinyin->ChineseModel;
    for(int i=googlePinyin->ChineseModel.size()-1; i>=0; i--)
    {
        tempChineseList.append(googlePinyin->ChineseModel.at(i));
    }
